...
export const applyWaveFunction = (x: number, y: number) => {
	...
	let updated =
		(2 * curr[idx] - prev[idx] * (1 - halfSigmaDt) + c2dt2 * laplacian) / (1 + halfSigmaDt);
...
export const gpuWaveFunction = (
	...
	const { pipeline, sizeUniform, constantsUniform } = once(gpuWaveFunction, () => {
	...
	pipeline
		.with(bindGroup)
		.dispatchWorkgroups(Math.ceil(options.width / 8), Math.ceil(options.height / 8));
};
...
export const renderWave = (
	...
	const { pipeline, sizeUniform } = once(renderWave, () => {
